...
// ─── ПРОКСИ ДЛЯ КАРТИНОК ─────────────────────────────────────────────────────
// Игроки загружают картинки через /img?url=… — сервер забирает с Wikipedia
// сам, браузеры не трогают Wikipedia напрямую → нет блокировок, нет лагов.
...
// Отдать из кэша если уже загружали
...
imageCache.set(url, buf); // кэшируем на время жизни сервера
...
// Превращаем оригинальный URL картинки в прокси-ссылку
...
const players = getPlayers(room);
if (!players.length) {
emitRoomMessage(roomCode, “Нет игроков для игры”);
return;
}
...
// Ведущий видит картинку через прокси — быстро и надёжно
if (room.hostId) {
...
```
// Прогреваем кэш заранее — сервер скачивает картинку сразу
// чтобы к моменту победы она уже была готова для всех игроков
if (room.currentCharacter.image) {
  https.get(room.currentCharacter.image, { headers: { "User-Agent": "Mozilla/5.0" } }, (res) => {
    const chunks = [];
    res.on("data", (c) => chunks.push(c));
    res.on("end", () => {
      imageCache.set(room.currentCharacter.image, Buffer.concat(chunks));
    });
  }).on("error", () => {});
}
```
...
if (socket.id !== room.hostId) {
  socket.emit("systemMessage", "Только ведущий отвечает на вопросы");
  return;
}
...
```
if (socket.id === room.hostId) {
  socket.emit("systemMessage", "Ведущий не может угадывать");
  return;
}
...
if (isCorrectAnswer(answer, room.currentCharacter.name)) {
  ...
  // Картинка уже закэширована — победный экран появится мгновенно
